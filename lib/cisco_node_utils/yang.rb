...
module Cisco
  ...
  def needs_something?(op, target, run)
    def nil_array(elt)
      elt.is_a?(Array) && elt.length == 1 && elt[0].nil?
    end
    def sub_elt(op, target, run)
      if target.is_a?(Hash) && run.is_a?(Hash)
        return hash_equiv?(op, target, run)
      elsif target.is_a?(Array) && run.is_a?(Array)
        return array_equiv?(op, target, run)
      else
        return !(target != run && !nil_array(target))
      end
    end
    def array_equiv?(op, target, run)
      n = target.length
      loop = lambda {|i|
        if i == n
          true
