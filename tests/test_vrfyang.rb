...
class TestVrfYang < CiscoTestCase
  ...
  def test_name_zero_length
    assert_raises(Cisco::ClientError, "Zero length name didn't raise ClientError") do
      VrfYang.new('')
    end
  end
  ...
  def test_name_too_long
    ...
    assert_raises(Cisco::ClientError,
                  'vrf name misconfig did not raise ClientError') do
      VrfYang.new(name)
    end
  end

  def test_shutdown
    v = VrfYang.new('test_shutdown')
    if platform == :ios_xr
  ...
  def test_description
    v = VrfYang.new('test_description')
